...
void Aura::RefreshDuration(bool withMods)
{
    ...
    if (withMods && caster)
    {
        ...
        if (duration > 0 && (caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE, m_spellInfo) || m_spellInfo->HasAttribute(SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC)))
...
void Aura::RefreshTimersWithMods()
{
    Unit* caster = GetCaster();
    m_maxDuration = CalcMaxDuration();
    if (m_maxDuration > 0 && ((caster && caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE, m_spellInfo)) || m_spellInfo->HasAttribute(SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC)))
    {
