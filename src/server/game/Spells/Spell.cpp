        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
                    ...
                    if (duration > 0 && (m_originalCaster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE, aurSpellInfo) || m_spellInfo->HasAttribute(SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC)))
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    if (didHit)
        if (int32 rp = int32(runeCostData->runePowerGain * sWorld->getRate(RATE_POWER_RUNICPOWER_INCOME) / sWorld->getRate(RATE_REGEN_POWER_RUNICPOWER_INCOME)))
